...
// Property and event names are interned once and reused for every message.
v8::Local<v8::String> Str(v8::Isolate* isolate, std::string_view s) {
  static base::NoDestructor<
      absl::flat_hash_map<std::string_view, v8::Eternal<v8::String>>>
      strings;
  auto it = strings->find(s);
  if (it == strings->end()) {
    // Keyed by the caller's storage: every caller passes a literal.
    it = strings
             ->emplace(s, v8::Eternal<v8::String>(
                              isolate, gin::StringToSymbol(isolate, s)))
             .first;
  }
...
void SendReply(v8::Local<v8::Context> context,
               ...
               v8::Local<v8::Value> value) {
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
  if (gin_helper::internal::ReplyChannel* channel =
          api::ReplyChannelOf(isolate, event)) {
    channel->SendReply(isolate, value);
  }
}

void ReturnValueGetter(v8::Local<v8::Name> name,
                       const v8::PropertyCallbackInfo<v8::Value>& info) {
  info.GetReturnValue().SetUndefined();
}

void ReturnValueSetter(v8::Local<v8::Name> name,
                       v8::Local<v8::Value> value,
                       const v8::PropertyCallbackInfo<v8::Boolean>& info) {
  v8::Isolate* isolate = info.GetIsolate();
  SendReply(isolate->GetCurrentContext(), info.Holder(), value);
}

// event.returnValue = x replies to a synchronous message; reads are always
// undefined. Own and non-enumerable.
void AddReturnValue(v8::Local<v8::Context> context,
                    v8::Local<v8::Object> event) {
  std::ignore = event->SetNativeDataProperty(
      context, Str(JavascriptEnvironment::GetIsolate(), "returnValue"),
      ReturnValueGetter, ReturnValueSetter, v8::Local<v8::Value>(),
      v8::DontEnum);
}
...
bool AddPorts(v8::Local<v8::Context> context,
  ...
  if (api::IpcMainEvent* e = api::IpcMainEvent::FromV8(isolate, event)) {
    e->SetPorts(isolate, array);
  } else if (api::IpcMainServiceWorkerEvent* sw =
                 api::IpcMainServiceWorkerEvent::FromV8(isolate, event)) {
    sw->SetPorts(isolate, array);
  } else {
    return false;
  }
  return true;
...
// What an invoke replies with once the handler's result has settled.
struct PendingInvokeReply {
  v8::Global<v8::Object> event;
  v8::Global<v8::Value> channel;
  v8::Global<v8::Value> value;
  bool rejected = false;
};

void SendInvokeReply(std::unique_ptr<PendingInvokeReply> pending) {
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
  v8::HandleScope handle_scope(isolate);
  v8::Local<v8::Object> event = pending->event.Get(isolate);
  v8::Local<v8::Context> context = event->GetCreationContextChecked(isolate);
  v8::Context::Scope context_scope(context);
  if (pending->rejected) {
    ReplyWithError(context, event, pending->channel.Get(isolate),
                   pending->value.Get(isolate));
  } else {
    ReplyWithResult(context, event, pending->channel.Get(isolate),
                    pending->value.Get(isolate));
  }
}

void SendInvokeReplyMicrotask(void* data) {
  SendInvokeReply(std::unique_ptr<PendingInvokeReply>(
      static_cast<PendingInvokeReply*>(data)));
}

// Replies in a microtask, where `await`ing the result used to put it, so
// microtasks the handler queued still run first.
void QueueInvokeReply(v8::Local<v8::Context> context,
                      v8::Local<v8::Object> event,
                      v8::Local<v8::Value> channel,
                      v8::Local<v8::Value> value,
                      bool rejected) {
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
  auto pending = std::make_unique<PendingInvokeReply>();
  pending->event.Reset(isolate, event);
  pending->channel.Reset(isolate, channel);
  pending->value.Reset(isolate, value);
  pending->rejected = rejected;
  context->GetMicrotaskQueue()->EnqueueMicrotask(
      isolate, SendInvokeReplyMicrotask, pending.release());
}

// Promise reactions for a result that has not settled yet; |data| is
// [event, channel].
void OnInvokeFulfilled(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  v8::Local<v8::Value> event, channel;
  if (data->Get(context, 0).ToLocal(&event) && event->IsObject() &&
      data->Get(context, 1).ToLocal(&channel)) {
    ReplyWithResult(context, event.As<v8::Object>(), channel, info[0]);
  }
}
...
void OnInvokeRejected(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (data->Get(context, 0).ToLocal(&event) && event->IsObject() &&
      data->Get(context, 1).ToLocal(&channel)) {
    ReplyWithError(context, event.As<v8::Object>(), channel, info[0]);
  }
}

// promise.then(reply, replyWithError)
void ReplyWhenSettled(v8::Local<v8::Context> context,
                      v8::Local<v8::Promise> promise,
                      v8::Local<v8::Object> event,
                      v8::Local<v8::Value> channel) {
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
  v8::Local<v8::Value> parts[] = {event, channel};
  v8::Local<v8::Array> data = v8::Array::New(isolate, parts, 2);
  ...
  std::ignore = promise->Then(context, on_fulfilled, on_rejected);
}

// Whether Promise.resolve(value) would return |value| itself: a Promise whose
// `constructor` is %Promise%. Instances of Promise subclasses are not; they
// are adopted through their own then() like any other thenable.
bool IsNativePromise(v8::Local<v8::Context> context,
                     v8::Local<v8::Value> value) {
  if (!value->IsPromise())
    return false;
  v8::Isolate* isolate = JavascriptEnvironment::GetIsolate();
  ...
  return value.As<v8::Object>()
             ->Get(context, Str(isolate, "constructor"))
             .ToLocal(&constructor) &&
         constructor->StrictEquals(
             GetRegistry().promise_constructor.Get(isolate));
}
...
void RunInvokeHandler(v8::Local<v8::Context> context,
  ...
  // await result, then reply. A plain value or a fulfilled Promise replies in
  // the next microtask without promise reactions; a pending or rejected one
  // gets a reaction (attaching it is also what tells the unhandled-rejection
  // tracker a rejection was handled). Any other object goes through a
  // resolver, which reads its `then` once and adopts it if it is a thenable.
  v8::Local<v8::Promise> promise;
  if (IsNativePromise(context, result)) {
    promise = result.As<v8::Promise>();
  } else if (try_catch.HasCaught()) {
    // Reading `constructor` threw, which Promise.resolve() turns into a
    // rejection.
    if (try_catch.HasTerminated())
      return;
    v8::Local<v8::Value> error = try_catch.Exception();
    try_catch.Reset();
    QueueInvokeReply(context, event, channel_value, error, /*rejected=*/true);
    return;
    ...
    QueueInvokeReply(context, event, channel_value, result, /*rejected=*/false);
    return;
  }
  if (promise->State() == v8::Promise::kFulfilled) {
    QueueInvokeReply(context, event, channel_value, promise->Result(),
                     /*rejected=*/false);
  } else {
    ReplyWhenSettled(context, promise, event, channel_value);
  }
}
...
void Setup(v8::Isolate* isolate, const gin_helper::Dictionary& options) {
  ...
  if (v8::Promise::Resolver::New(context).ToLocal(&resolver) &&
      resolver->GetPromise()
          ->Get(context, gin::StringToSymbol(isolate, "constructor"))
          .ToLocal(&constructor)) {
    registry.promise_constructor.Reset(isolate, constructor);
  }
}
